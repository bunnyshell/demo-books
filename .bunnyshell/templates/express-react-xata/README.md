...
This Environment [Template](https://documentation.bunnyshell.com/docs/templates-what-are-templates) is a boilerplate for creating a new environment based on a stack using Node.js with Express for the backend, React with Typescript for the frontend and xata.io as the database.

The template provides the Bunnyshell configuration composed of 3 Components (frontend + backend + database management) and the CRUD application that demonstrates how the components work together to form an environment.
